...
export default function ConsultantSidebar({ isOpen, onClose }: ConsultantSidebarProps) {
  const pathname = usePathname();

  const navItems = [
    { href: '/consultant', label: 'Dashboard', icon: '📊' },
    { href: '/consultant/profile', label: 'Profile', icon: '👤' },
    { href: '/consultant/bookings', label: 'Bookings', icon: '📅' },
    { href: '/consultant/sessions', label: 'Sessions', icon: '💼' },
    { href: '/consultant/messages', label: 'Messages', icon: '💬' },
    { href: '/consultant/documents', label: 'Documents', icon: '📄' },
    { href: '/consultant/settings', label: 'Settings', icon: '⚙️' },
  ];

  const isActive = (href: string) => {
    if (href === '/consultant') {
      return pathname === '/consultant';
    }
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="px-6 py-4 border-t border-primary-800">
            <p className="text-xs text-primary-400">Consultant Portal</p>
          </div>
        </div>
      </aside>
    </>
