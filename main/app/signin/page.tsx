...
function SignInForm() {
  ...
  // Clear old tokens if redirected due to verification failure or on page load
  useEffect(() => {
    ...
    // Also clear tokens if we're on signin page and have invalid tokens
    // This helps when the API server was restarted with a new secret
    const token = localStorage.getItem('accessToken');
    if (token) {
      // Try to verify the token by checking if it's valid
      // If API server was restarted, old tokens won't work
      // We'll let the middleware handle verification and redirect back if needed
    }
  }, [searchParams]);
